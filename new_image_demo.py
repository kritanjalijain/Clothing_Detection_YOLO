...
model = 'yolo' #, 'retinanet' , 'maskrcnn', 'trident' 
...
#                     crop_path = "output/cropped/upper/" + str(img_id) + str(classes[int(cls_pred)])+ '.jpg' 
#                 else:
#                     print("Not cropping since not a top")
#                 #if classes[int(cls_pred)] in ['boots' , 'footwear']:
#                 #    crop_path = "output/cropped/footwear/" + str(img_id) + str(classes[int(cls_pred)])+ '.jpg' 
...
#                 # else:
#                 #     parent_dir = '/home/kritanjali/Desktop/Internship/Clothing-Detection/output/cropped/'
...
#                 #         print(error)
#                 #     crop_path = new_dir_path + "/" + str(img_id) + str(classes[int(cls_pred)])+ '.jpg' 
#                 if((x1 > 0) & (x2 > 0) & (y1 > 0) & (y2 > 0)):
#                     cv2.imwrite(crop_path,img_crop)
#                 cv2.rectangle(img,(x1,y1) , (x2,y2) , color,3)
#                 y1 = 0 if y1<0 else y1
...
#         img_id = path.split('/')[-1].split('.')[0]
#         print(img_id)
#         output_path = 'output/output-test_{}_{}_{}.jpg'.format(img_id,model,dataset)
#         print(output_path)
